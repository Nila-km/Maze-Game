...
function genSides() {
  ...
  nogoY2.push(l1 + 2 * step, l2 + 2 * step, l1 + step + bwidth, l1 + 2 * step + bwidth, l2 + step + bwidth, l2 + 2 * step + bwidth);
  //set start-pos
...
// tambahan warna
const textContainer = document.getElementById("judul");
...
function startGame() {
  ...
  if (!gameStarted) {
    ...
    // Additional logic or functions needed when the game starts
  }
}
...
function updateTimer() {
  ...
  score += 10; // Update score based on your game logic
  document.getElementById("score-value").innerText = score; // Update the displayed score
  document.getElementById("timer-value").innerText = formatTime(elapsedSeconds);

  // Additional logic for updating the score and checking win condition
  checkWinCondition();
}
...
function resetGame() {
  ...
  startTime = 0;
  score = 0;
  document.getElementById("score-value").innerText = "0"; // Reset displayed score
  document.getElementById("timer-value").innerText = "0:00"; // Reset displayed time
  // Additional logic for resetting the game, such as resetting character position, score display, etc.
}

function checkWinCondition() {
  // Check if the player has reached home
  const playerLeft = thingie.offsetLeft;
  const playerTop = thingie.offsetTop;
  const homeLeft = home.offsetLeft;
